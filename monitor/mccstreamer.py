...
def get_rolling_data(buffer, last_index, current_index):
    """
    Extracts the part of the __data that is between last_index and index
    This function is able to deal with a rolling buffer where new __data can
...
class MCCStreamer(Streamer):
    def __init__(self) -> None:
        ...
        self._sorted_queue, self._row_pos = zip(*sorted(zip(self._queue_list, self._row_pos), key=lambda x: x[0].channel))

        self._ai_device.a_in_load_queue(self._sorted_queue)
        self.__data = create_float_buffer(len(chan_list), self.bufferSz)
        self._last_index = 0
