...
class SamplingThread(threading.Thread):
    def __init__(self, aiConfig, bufferSize=10000):
        ...
        self._dev = comedi.comedi_open(str(self._aiConfig['device']))
        if not self._dev:
            logging.error("FATAL ERROR: cannot open comedi device %s: %s",
                          (self._aiConfig['device'], comedi.comedi_strerror(comedi.comedi_errno())))
            raise IOError()
        ...
        # create channel list
        myChanList = comedi.chanlist(int(self._aiConfig['self.nbChans']))
        for i in range(int(self._aiConfig['self.nbChans'])):
            myChanList[i] = comedi.cr_pack(self._aiConfig['channels'][i],
        ...
        for i in range(2):
            ret = comedi.comedi_command_test(self._dev, cmd)
            if ret < 0:
        ...
        # Start the command
        ret = comedi.comedi_command(self._dev, cmd)
        if ret != 0:
