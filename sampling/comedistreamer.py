...
class ComediStreamer(Streamer):
    def __init__(self, config, bufferSize=10000, start=True):
        ...
        if not self._dev:
            logging.error(
                "FATAL ERROR: cannot open comedi device: %s",
        ...
        for channel in self._config["channels"]:
            self.__channelMaxValues.append(
                comedi.comedi_get_maxdata(
                    self._dev, self._subdevice, channel["channel-id"]
                )
            )
            temp = comedi.comedi_get_range(
                self._dev, self._subdevice, channel["channel-id"], channel["gain"]
            )
        ...
        if self._fd <= 0:
            logging.error(
                "Error obtaining comedi device file descriptor: %s",
                comedi.comedi_strerror(comedi.comedi_errno()),
            )
            raise IOError(comedi.comedi_strerror(comedi.comedi_errno()))
        ...
        for i, channel in enumerate(self._config["channels"]):
            myChanList[i] = comedi.cr_pack(
                channel["channel-id"], channel["gain"], channel["ref"]
            )
        # create a command structure
        self._cmd = comedi.comedi_cmd_struct()
        ret = comedi.comedi_get_cmd_generic_timed(
            self._dev,
            self._subdevice,
            self._cmd,
            self.nbChannels,
            int(1.0e9 / self._config["sample-rate"]),
        )
        if ret:
    ...
    def to_physical(self, inData):
        ...
        out *= np.reshape(
            (
                np.array(self.__channelRangeMaxValues)
                - np.array(self.__channelRangeMinValues)
            ),
            (self.nbChannels, 1),
        )
